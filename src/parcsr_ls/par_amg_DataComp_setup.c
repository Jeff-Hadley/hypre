...
/*****************************************************************************
 *
 * Routine for driving the setup phase of AMG Data Compression
 * 
 * SEARCH FOR 'XXX' to find comments about stuff that likely needs removing
 * and confirmation that it needs removal. 
 *
 *****************************************************************************/
...
HYPRE_Int
hypre_BoomerAMGDataCompSetup( void               *amg_vdata,
                      ...
                      hypre_ParVector    *u )
{
   ...
   /* Setting new local values to values stored in amg_data */
   {
   ...
   /* Linking Arrays produced by coarsening */
   {
   S = NULL;
   ...
   /* Setting the relax type for the coarse grid - We don't want to relax at any point
   See if we even need this call. XXX */
 ...
 /* change in definition of standard and multipass interpolation, by
   ...
   /* Allocating storage for A, P, R, Coarse-Fine arrays */
   {
   if (A_array == NULL)
   {
   ...
   /* Setting A, P, R of solver object to allocated memory */
   {
   if (block_mode)
   {
   ...
   /* Relaxation set-ups - Probably don't need this bit... confirm before removing XXX */
   {
   ...
   /* Allocating memory for F and U, reseting memory if already set */
   {
  ...
  /*----------------------------------------------------------
   ...
   max_row_sum = hypre_ParAMGDataMaxRowSum(amg_data); // Probably want this to be 1
   ...
   P_max_elmts = hypre_ParAMGDataPMaxElmts(amg_data); //Look into what this is.
   ...
   smooth_num_levels = hypre_ParAMGDataSmoothNumLevels(amg_data); // Probably want this to be 0 if don't want smoothing
   ...
   while (not_finished_coarsening)
   {
      ...
      else /* max_levels > 1 */
      {
         ...
         if (hypre_ParAMGDataGSMG(amg_data) == 0)
         {
            ...
            // Won't be used. Restriction type is 0, R = P^T
         ...
         else /* If using GSMG, which we are not*/
         {
         ...
         /* We don't use this, all points are to be considered in solving Coarse/Fine */ 
         ...
         if (nodal == 0) /* no nodal coarsening */
         {
            ...
            /* Aggresive coarsening is performed here  - if current level is in our range of aggressive coarsening levels */
            /* Creates a second S that is a copy(?) of S, calculates SofC of S, 
            then stores the CF markers in CFN_marker, and destroys the second S it created*/
         ...
         /* here we will determine the coarse grid size to be able to
            ...
            /* If it's smaller than the minimal size, stop looping, set 
            current fine level as the ending coarse level*/
         ...
         if (level >= agg_num_levels)
         {
             ...
             * coarsest solve to be a single sweep of default smoother or smoother set by user */
             ...
             /* Setting of the smoothers probably can go since we won't actually go solve this. */
             /* This bit is determining if we can stop the coarsening loop */
         ...
         /* If doing aggressive coarsening */
         if (level < agg_num_levels)
         {
            if (nodal == 0)
            {
         ...
         else /* no aggressive coarsening */
         {
            ...
            /* RL: build restriction - If non-Galerkin */
            if (restri_type)
            {
            ...
            /* Determine our interpolation type */
               ...
               else /* not block mode - use default interp (interp_type = 0) */
               {
                  if (nodal > -1) /* non-systems, or systems with unknown approach interpolation*/
                  {
                     ...
                     else /* one function only or unknown-based interpolation- */
                     {
                        ...
                        /* This is the one we will end up using - P is what we will then need to perform energy minimization smoothing on. */
                        hypre_BoomerAMGBuildInterp(A_array[level], CF_marker,
...
/* THIS IS WHERE WE WILL WANT TO SMOOTH P USING THE ENERGY MINIMIZATION */
      ...
      /* Do something similar as what is done above ^^ Just have a flag set somehwere*/
      ...
      if (!block_mode)
      {
         // If doing additive V-cycle and are in that range (We are not)
         ...
         else //Store P from this level into the P_array
         {
            P_array[level] = P;
            /* RL: save R matrix */
            if (restri_type)
      ...
      else if (mult_addlvl == -1 || level < mult_addlvl || level > add_end)
      {
         ...
         else
         {
            ...
            else //This is the one we will do 
            {
      ...
      if (!block_mode)
      {
         /* dropping in A_H */ // XXX Look at what tol to set for this
